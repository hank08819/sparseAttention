"""
train.py — Training and evaluation utilities for MSCA-SBiGRU.

Usage::

    from msca_sparsity import MSCABiGRU, Trainer

    model   = MSCABiGRU(feat_dim=7)
...
def get_device(preference: str = "auto") -> str:
    """Return the best device for MSCA training.
    ...
    preference : {"auto", "cpu", "cuda", "mps"}
        ``"auto"``  → CUDA if available, else CPU.
                      ...
                      for MSCA's small hidden=64, batch=64 workload because
...
class Trainer:
    """Wraps MSCABiGRU training with cosine LR schedule and early stopping.
...
def dm_test(e1: np.ndarray, e2: np.ndarray, horizon: int = 5):
    ...
    ----------
    e1 : np.ndarray
        Forecast errors of model 1 (e.g. MSCA-SBiGRU).
    e2 : np.ndarray
