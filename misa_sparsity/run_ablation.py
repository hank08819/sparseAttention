"""
run_ablation.py — Ablation study for MISA
==================================================
Trains 4 model variants per asset-period pair (seed 42 only for speed):

  FULL  : full MISA (sparsemax + multi-scale + cross-asset gate)
...
from misa_sparsity import MISABiGRU, Trainer, build_features
from misa_sparsity.train import dm_test
...
def train_variant(splits, kwargs, seed=42):
    ...
    model = MISABiGRU(feat_dim=splits["F"], hidden=64, n_heads=4,
